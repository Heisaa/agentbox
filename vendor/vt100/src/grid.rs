...
impl Grid {
    ...
    pub fn scroll_up(&mut self, count: u16) {
        for _ in 0..(count.min(self.size.rows - self.scroll_top)) {
            ...
            // Local patch (agentbox): also preserve lines scrolled off a
            // top-anchored scroll region, not just full-screen scrolls.
            // Inline TUIs such as codex insert transcript history by
            // scrolling inside a `CSI 1;N r` region; dropping those rows
            // would lose the conversation history. The alternate screen is
            // unaffected because its grid has scrollback_len == 0.
    ...
    // Unused since the local scrollback patch in scroll_up, kept to
